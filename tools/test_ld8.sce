...
try
    ...
    assert_checkequal(LD8.student.bank,"LD8-64-A-2026"); delete(LD8.fig);
    ...
    for dimension=1:3
        geometry_size(LD8.fig,sizes(dimension,:));
        for step=1:6
            ...
            if dimension==2 & step>=2 then capture_ld8("E"+string(step)); end
        end
    end
    ...
    stages=[1 3 4];
    for mode=1:3
        LD8.step=stages(mode); LD8.wireMode=mode; LD8.wires=ld8_canonical_wires(mode);
        LD8.powerOn=%t; LD8.switchOn=%t; ld8_render_stage(); values=ld8_reference(mode);
        ...
        rows=ld8_journal_rows(mode);
        assert_checkalmostequal(rows(1,1:2),values(1:2),1e-9,1e-9);
        if mode==3 then
            geometry_size(LD8.fig,[1280 800]); execstr(LD8.fig.resizefcn); capture_ld8("misri-V64");
        end
    end
    ...
    mputl("LD8_PASS: 3 GUI variants; series/parallel/mixed wirings; true MNA readings; actual report button; saved wiring evidence; comma input without Enter; demo isolation; draft restore; 39 geometry cases including resize callback; 144 grading comparisons",out+"verdict.log"); exit(0);
catch
    mputl("LD8_FAIL: "+strcat(lasterror()," | "),out+"verdict.log"); disp(lasterror()); exit(1);
end
