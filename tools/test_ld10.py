#!/usr/bin/env python3
"""Internal feedback only: bounded native Scilab LD10 UI acceptance on both OSes."""
import argparse
import hashlib
import re
import json
...
env = dict(os.environ, LD10_TEST_RUNTIME=runtime.as_posix(), LD10_TEST_OUT=out.as_posix(),
           LD10_TEST_SOURCE=repo.as_posix(), LD_DATA_DIR=(out / 'Ataskaitos Žąsė').as_posix())
...
args = [str(exe), *mode, '-nb', '-f', str(repo / 'tools/test_ld10.sce')]
...
assert result.returncode == 0 and 'LD10_PASS:' in verdict, verdict
print(verdict.strip())
...
print(result.stdout)
assert result.returncode == 0, 'LD10 geometry failed'
