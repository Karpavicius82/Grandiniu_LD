#!/usr/bin/env python3
"""Internal feedback only: bounded native Scilab LD5 UI acceptance on both OSes."""
import argparse
import json
import sys
import os
...
env = dict(os.environ, LD5_TEST_RUNTIME=runtime.as_posix(), LD5_TEST_OUT=out.as_posix(),
           LD5_TEST_SOURCE=repo.as_posix(), LD_DATA_DIR=(out / 'Ataskaitos Žąsė').as_posix())
...
args = [str(exe), *mode, '-nb', '-f', str(repo / 'tools/test_ld5.sce')]
...
assert result.returncode == 0 and 'LD5_PASS:' in verdict, verdict
print(verdict.strip())
...
print(result.stdout)
assert result.returncode == 0, 'LD5 geometry failed'
...
for index, case in enumerate(cases):
    ...
    expected[name] = 16 if case['accepted'] else 15
...
for result in verdicts['results']:
    assert result['status'] == 'graded' and result['max_points'] == 16, result
...
for result in reports:
    assert result['status'] == 'graded' and (result['points'], result['max_points']) == (16, 16), result
...
summary = dict(status='PASS', tolerance_cases=len(expected), actual_gui_reports=3,
               variants=[1, 17, 64], geometry_cases=36, platform=os.name)
