#!/usr/bin/env python3
"""Internal feedback only: bounded native Scilab LD8 UI acceptance on both OSes."""
import argparse
import hashlib
import re
import json
...
print(result.stdout)
assert result.returncode == 0, 'LD8 geometry failed'
...
for index, case in enumerate(cases):
    ...
    expected[name] = 21 if case['accepted'] else 20
...
summary = dict(status='PASS', tolerance_cases=len(expected), actual_gui_reports=3,
               variants=[1, 17, 64], geometry_cases=19, assessment_reports_selected=True, close_autosave_restore=True, edge_cases=True, malformed_drafts_rejected=6, report_traceability=True,
