#!/usr/bin/env python3
"""Internal feedback only: bounded native Scilab LD7 UI acceptance on both OSes."""
import argparse
import json
import sys
import os
...
env = dict(os.environ, LD7_TEST_RUNTIME=runtime.as_posix(), LD7_TEST_OUT=out.as_posix(),
           LD7_TEST_SOURCE=repo.as_posix(), LD_DATA_DIR=(out / 'Ataskaitos Žąsė').as_posix())
...
args = [str(exe), *mode, '-nb', '-f', str(repo / 'tools/test_ld7.sce')]
...
assert result.returncode == 0 and 'LD7_PASS:' in verdict, verdict
print(verdict.strip())
...
print(result.stdout)
assert result.returncode == 0, 'LD7 geometry failed'
...
for index, case in enumerate(cases):
    ...
    expected[name] = 27 if case['accepted'] else 26
...
for result in verdicts['results']:
    assert result['status'] == 'graded' and result['max_points'] == 27, result
