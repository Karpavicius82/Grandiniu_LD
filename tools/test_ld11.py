...
env = dict(os.environ, LD11_TEST_RUNTIME=runtime.as_posix(), LD11_TEST_OUT=out.as_posix(),
           LD11_TEST_SOURCE=repo.as_posix(), LD_DATA_DIR=(out / 'Ataskaitos Žąsė').as_posix())
...
args = [str(exe), *mode, '-nb', '-f', str(repo / 'tools/test_ld11.sce')]
...
print(result.stdout)
assert result.returncode == 0, 'LD11 geometry failed'
...
for index, case in enumerate(cases):
    ...
    expected[name] = 20 if case['accepted'] else 19
...
for result in verdicts['results']:
    assert result['status'] == 'graded' and result['max_points'] == 20, result
