...
with zipfile.ZipFile(root / "dist/Grandiniu_LD-studentui.zip") as archive:
    ...
    runtime = {str(path.relative_to(student)) for path in student.rglob("*")
               if path.is_file() and path.suffix in {".sci", ".sce", ".sh", ".bat", ".command"}}
    ...
    for name in ["README.md", "LD1/VARIANTAI.csv", "LD2/VARIANTAI.csv", "LD3/VARIANTAI.csv","LD4/VARIANTAI.csv","LD5/VARIANTAI.csv","LD6/VARIANTAI.csv","LD7/VARIANTAI.csv","LD8/VARIANTAI.csv","LD9/VARIANTAI.csv","LD10/VARIANTAI.csv","LD11/VARIANTAI.csv","LD12/VARIANTAI.csv", "capture_window.py"]:
