// ============================================================================
// LD9 variantų konfigūracija (bankas LD9-64-A-2026). Nuosekliai sujungtos RLC
...
function cfg = ld9_variant_config(number)
    if ~ld9_valid_index(number, 64) then error("Eilės numeris turi būti nuo 1 iki 64."); end
    ls = [10;12;15;18;22;27;33;39]; cs = [47;56;68;82;100;120;150;180];
    row = floor((number-1)/8)+1; column = modulo(number-1, 8)+1;
    l = ls(row)*1e-3; c = cs(column)*1e-9;
    qt = 2 + 0.5*modulo(row-1+column-1, 4);
    cfg = struct("E", 5, "L", l, "C", c, ...
        "R", round(100*sqrt(l/c)/qt)/100, "LmH", ls(row), "CnF", cs(column));
endfunction

function [valid, why] = ld9_validate_config(cfg)
    ...
    if abs(cfg.R - round(100*sqrt(cfg.L/cfg.C)/qt)/100) > 1e-9 then return; end
    valid = %t; why = "";
endfunction
