...
function [term_xy,boxes]=ld6_layout()
    ...
    boxes=struct("E1",[7 63 15 16],"K",[38.5 65 12 14],"A",[66 63 14 16], ...
        "R",[18 39 19 14],"E2",[54 39 19 14],"V",[54 12 19 16]);
    term_xy=struct("E1_N",[4.5 72],"E1_P",[24.5 72], ...
        "K1",[35.5 72],"K2",[53.5 72],"A_P",[63.5 72],"A_N",[83 72], ...
        "R_A",[15 46],"R_B",[40 46],"E2_P",[51 46],"E2_N",[76 46], ...
        "V_P",[51 20],"V_N",[76 20]);
endfunction
...
function ld6_render_wires()
    ...
    for k=1:size(LD6.wires,1)
        ...
        elseif (a=="E1_N" & b=="R_B") | (b=="E1_N" & a=="R_B") | (a=="E1_N" & b=="E2_N") | (b=="E1_N" & a=="E2_N") then
        ...
        elseif (a=="R_B" & b=="E2_P") | (b=="R_B" & a=="E2_P") then
    ...
    names=["E1 ŠALTINIS" "JUNGIKLIS" "AMPERMETRAS" "R KROVINYS" "E2 ŠALTINIS" "VOLTMETRAS"];
    switchText="Atviras"; if LD6.switchOn then switchText="Uždarytas"; end
    powerText="Išjungtas"; if LD6.powerOn then powerText="9 V"; end
    [u,i,valid,reason]=ld6_measure_values();
    ampText="— mA"; voltText="— V";
    if valid then ampText=msprintf("%.3f mA",i); voltText=msprintf("%g V",u); end
    rvText=msprintf("%g V",LD6.cfg.E2);
    if ld6_valid_index(LD6.wireMode,3) then
    end
    vals=[powerText switchText ampText msprintf("%g Ω",LD6.cfg.R) rvText voltText];
    ...
    for k=1:6
        ...
        if k==5 then h.tooltipstring=msprintf("Antrasis šaltinis E2 = %g V.",LD6.cfg.E2); end
    end
    ...
    for id=matrix(tids,1,-1)
        ...
        if LD6.demoMode | LD6.step<>1 then h.enable="off"; end
    ...
    LD6.ui.controls(6).enable="off";
    if ~LD6.demoMode & or(LD6.step==[2 3 4]) then LD6.ui.controls(6).enable="on"; end
    ld6_font(p);
...
function ld6_build_gui()
    ...
    f.figure_name="LD6 · Šaltinių jungimas"; f.background=color(246,248,249); LD6.fig=f;
    LD6.ui=struct("headless",%f,"boardHandles",list(),"dynamic",[],"controls",[]);
    LD6.term=struct("handles",list(),"handleIds",emptystr(0,1));
    student_text(f,[0.03 0.925 0.65 0.05],"LD6  /  Šaltinių jungimas",22,%t,[0.965 0.973 0.977]);
    ...
    LD6.ui.journalList=uicontrol(p,"style","listbox","units","normalized","position",[0.04 0.82 0.73 0.15], ...
        "string","Matavimai","fontname","DejaVu Sans","fontunits","pixels","fontsize",14,"tag","V02");
    controls=[];
    lbl=["E1";"E1+E2";"E1−E2"];
    for k=1:3
        cb="ld6_set_mode("+string(k)+")";
        controls($+1)=ld6_button(p,[0.02+(k-1)*0.165 0.025 0.155 0.060],lbl(k),cb);
    end
