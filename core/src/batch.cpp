...
public:
    ...
    void finish(bool cancel=false) {
        ...
        html+="<p>Nuskaityta "+std::to_string(cursor)+" iš "+std::to_string(files.size())+" failų. "+(cancelled?"Vertinimas sustabdytas.":"Vertinimas baigtas.")+"</p><p>Rubrika: LD1-1 / LD2-1 / LD3-1 / LD4-1 / LD5-1. Balas = 10 × surinkti taškai / visi taškai; apvalinama iki 0,1. Suvestinei parenkamas geriausias to paties studento to paties darbo bandymas. Visi bandymai pateikti žemiau.</p>";
        for(auto& r:results) {
