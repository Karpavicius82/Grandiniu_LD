// Bounded complex modified nodal analysis, ABI 1.
#include <Eigen/Dense>
#include <cmath>
#include <complex>
#include <vector>
...
LD_EXPORT void ld_mna(const double* table, const int* parts,
    ...
    *status=1;
    ...
    if (m<1 || m>256 || n<1 || n>64 || !std::isfinite(*frequency) || *frequency<0 || *frequency>1e9) return;
    std::fill(voltage,voltage+(n+1)*2,0.0);
    std::fill(current,current+m*2,0.0);
    try {
        ...
        for (int k=0;k<m;++k) {
            ...
            if (q.kind<=3 && (q.value.imag()!=0 || q.value.real()<1e-15 || q.value.real()>1e15)) return;
            if (std::abs(q.value)>1e15) return;
        ...
        // Equilibrate rows and columns before testing conditioning. This avoids
        // comparing raw conductances with dimensionless voltage constraints.
        Eigen::MatrixXcd scaled=A;
        Eigen::VectorXcd rhs=b;
        for(int k=0;k<scaled.rows();++k) {
            double s=scaled.row(k).cwiseAbs().maxCoeff();
            if(s==0) {*status=2; return;}
            scaled.row(k)/=s; rhs(k)/=s;
        }
        Eigen::VectorXd columns(scaled.cols());
        for(int k=0;k<scaled.cols();++k) {
            columns(k)=scaled.col(k).cwiseAbs().maxCoeff();
            if(columns(k)==0) {*status=2; return;}
            scaled.col(k)/=columns(k);
        }
        Eigen::FullPivLU<Eigen::MatrixXcd> lu(scaled);
        if (!lu.isInvertible() || lu.rcond()<1e-14) {*status=2; return;}
        Eigen::VectorXcd x=lu.solve(rhs).cwiseQuotient(columns.cast<C>());
